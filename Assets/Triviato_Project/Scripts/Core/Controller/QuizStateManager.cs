...
public class QuizStateManager
{
    ...
    public void Initialize(Quiz quizData, int numberOfQuestionsToUse = -1)
    {
        this.quizData = quizData;
        RandomizeQuestions(numberOfQuestionsToUse);
        ResetState();
    }
    ...
    private void RandomizeQuestions(int numberOfQuestionsToUse = -1)
    {
        ...
        if (numberOfQuestionsToUse > 0 && numberOfQuestionsToUse < randomizedQuestions.Count)
        {
            randomizedQuestions = randomizedQuestions.GetRange(0, numberOfQuestionsToUse);
        }
    }
}
